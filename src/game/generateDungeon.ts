...
export function createDungeonLevel(seed: CampaignSeed): DungeonState {
  ...
  for (let index = 0; index < 5; index += 1) {
    ...
    const width = randomInteger(random, 3, 4);
    const height = randomInteger(random, 3, 4);
    const x = bandStart + randomInteger(random, 0, 4 - width);
    const upperRoom = index % 2 === 1;
    const y = upperRoom
      ? randomInteger(random, 1, Math.max(1, 4 - height + 1))
      : randomInteger(random, 7, Math.max(7, ROWS - height - 1));
