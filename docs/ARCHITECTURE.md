...
Typed hydration presentation now distinguishes incompatible Dungeon campaigns from corruption, migration, unavailable storage, and write/verification failures. The explicit incompatible-replacement operation lives at the application/persistence boundary: after confirmation it migrates compatible campaigns, omits only campaigns proven incompatible, writes and verifies the version-2 candidate, and never writes to the legacy adapter. Confirmations and sheets reuse `ModalOverlay` for focus containment and restoration. The development inspector remains read-only while exposing the version-5 capital, World provenance/snapshot, contexts, retained Dungeon, and compatibility metadata.
