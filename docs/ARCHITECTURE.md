...
At the E03-T05 checkpoint, `src/game/campaignState.ts` introduced an explicit `CampaignStateV5` target contract beside the then-current application-facing version-4 compatibility type. E03-T06 subsequently made version 5 the application/save authority while retaining version 4 only for migration compatibility. The target contains only the accepted Castle foundation, Hero compatibility/build facts, capital Village, authoritative seven-region World snapshot, regional Dungeon snapshot/context, and lifecycle/resume metadata. Reusable definitions remain in the content catalogs, generated placement remains owned by the E03-T04 generator, and no economy or future gameplay slice is present.

`src/game/campaignMigration.ts` is the pure strict decode/migration boundary. It accepts untrusted campaign data plus the expected registry-owner reference; returns a typed success, validation failure, unsupported-version failure, incompatible-faction result, or deterministic-generation failure; and never reads or writes persistence. Supported v2/v3 sources pass through the exported protected v4 normalizer in `createGame.ts` before class-specific v5 conversion. Target v5 input is validated and cloned without generator calls, making ordinary target normalization idempotent.
...
`src/game/lifecycleV2.ts` and `registryCutover.ts` keep container transaction policy outside React and browser APIs. Hydration prefers a strict version-2 container, isolates campaign-level failures by profile, and preserves all valid entries in the typed working registry. When no campaign issue exists, legacy hydration delegates each campaign to the pure migration chain, persists and verifies a version-2 candidate, and never deletes or rewrites the source key. Candidate and readback bytes must both decode and validate; exact byte equality alone is insufficient. Typed failure prevents empty-registry autosave. `storage.ts` owns the two concrete local-storage keys at the platform edge.

`GameProvider` remains the application coordinator: it supplies clock/identity/seed/storage ports, invokes named pure operations, stamps successful campaign changes, performs verified writes, and exposes bounded player-visible persistence status. While campaign issues exist it blocks ordinary registry writes, so an isolated/filtered working registry cannot overwrite the source. Explicit confirmed recovery builds fresh pre-Setup Castle campaigns for affected profiles and commits them only after verified persistence; hard unreadable-container reset follows the same explicit verified boundary. It has no authority to create a partial completed foundation. The UI consumes the pure navigation policy; the former Dungeon Heart claim remains a compatibility-shaped provider operation returning `operation-retired`, and no board can use it to create or unlock the capital.
...
Typed hydration presentation distinguishes incompatible Dungeon campaigns from campaign corruption, migration, container corruption, unavailable storage, and write/verification failures. Campaign-level problems leave unaffected profiles usable and offer one explicit confirmed fresh-Castle recovery for affected profiles; hard parse/container failures offer an explicit reset, while unavailable storage remains retry-only. Neither path changes in-memory authority or reports success before the preferred payload passes candidate validation, write, exact readback, decode, and validation; the legacy adapter is never written. Confirmations and sheets reuse `ModalOverlay` for focus containment and restoration. The development inspector remains read-only while exposing the version-5 capital, World provenance/snapshot, contexts, retained Dungeon, and compatibility metadata.
...
Shared modal sheets use the small `ModalOverlay` presentation primitive for dialog semantics, title association, Escape/backdrop dismissal, initial focus, keyboard focus containment, opener-focus restoration, and background scroll locking. `SettingsSheet`, confirmation dialogs, and development inspection consume it. This is a reusable interaction foundation, not a global modal state machine. The former quick-information `HeroSheet` is retired by DMCL-029 because the implemented Hero board is now the single Hero information surface. `NotificationRegion` similarly provides only an always-mounted live region and presentation slot; campaign-saved feedback is its first consumer, with no queue or game-event ownership.
...
Hydration is a staged read/decode/validate/migrate operation. A failed stage suppresses automatic writes and cannot produce a legitimate empty replacement. Campaign-scoped failures are isolated without allowing the filtered result to persist implicitly. The original serialized payload remains intact until an explicitly authorized candidate replacement is validated, written, read back, decoded, and validated. A later write failure leaves the in-memory campaign explicitly unsaved and retryable.
