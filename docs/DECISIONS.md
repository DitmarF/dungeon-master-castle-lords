...
| DMCL-I05 | The E03-T06 Candidate uses `GameSave` version 5 inside `GameRegistry` version 2; strict legacy decoding/migration reads version 1 without modifying or deleting its payload. | Owner acceptance is required before checkpointing; retain the version-1 source throughout EPIC 03 under DMCL-P31. | `GAME_STATE.md`, `E03-T06_VILLAGE_FIRST_HERO_SETUP_TRANSITION.md` |
...
| DMCL-Q16 | **Resolved historically; superseded by current roadmap status** | E00-T05 activated EPIC 01 with E01-T01 next. EPIC 01 and EPIC 02 are now complete; EPIC 03 is Current and E03-T01 is the exact next task. |
...
On 2026-08-14, the project owner accepted DMCL-P26, the complete E03-T01 owner-decision packet recorded as DMCL-P27–DMCL-P39, and version 5 as the next campaign-schema number under DMCL-P40. E03-T05 implemented and the owner accepted the pure version-5 target/migration boundary. E03-T06 now presents the Candidate application/Setup cutover and activates the DMCL-P31 version-2 registry path because a separate verified container is required; the version-1 source remains untouched. No decision status changes in E03-T06.
...
DMCL-P27–DMCL-P39 now establish the browser-local one-campaign, verified Save, timestamp, replacement/deletion, recovery, opening, and migration-transition targets. Clock isolation and the accepted lifecycle behavior remain unimplemented until E03-T02; no deferred numeric gameplay question is promoted by this documentation checkpoint.
